...
local function PlayInstrumentAnimation(Humanoid, Playing, Idle, SpeedTrue, SpeedFalse, Bool)
	...
	if Bool then
		LoadedIdle:AdjustWeight(1)
		LoadedPlaying:AdjustWeight(10)
		LoadedPlaying:Play()
		LoadedPlaying.Looped = true
		LoadedIdle:Stop()
		LoadedPlaying:AdjustSpeed(SpeedTrue)
	else
		LoadedIdle:AdjustWeight(10)
		LoadedPlaying:AdjustWeight(1)
		LoadedPlaying:Stop()
		LoadedIdle:Play()
...
function ACS:PlayAvatarAnimation(Instrument, Bool)
	...
	if Bool then
		PlayInstrumentAnimation(Humanoid, Playing, Idle, 1, 0.4, Bool)
	else
		PlayInstrumentAnimation(Humanoid, Playing, Idle, 1, 0.4, Bool)
	end
end
